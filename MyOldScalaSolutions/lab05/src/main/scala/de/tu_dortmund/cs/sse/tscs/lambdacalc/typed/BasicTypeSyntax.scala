...
import de.tu_dortmund.cs.sse.tscs.lambdacalc._
...
trait BasicTypeSyntax extends LambdaCalculusSyntax{
  ...
  def TypeInfo = rule {
    TopTypeInfo | BaseTypeInfo | BoolTypeInfo | FunctionTypeInfo
  }
  ...
  def TopTypeInfo : Rule1[TypeInformation] = rule {
    capture("Top") ~> BaseTypeInformation
  }
