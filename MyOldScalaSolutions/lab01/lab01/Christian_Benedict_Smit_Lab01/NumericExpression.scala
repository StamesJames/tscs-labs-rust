...
trait NumericExpression extends Evaluation {
  ...
  override def -->(expr: Expression): Expression = {
    expr match {
      ...
      // Rules of B-Language
      case IfExpr(nestif, iftrue, iffalse) => if (progressPossible(nestif) && !isFinalResult(nestif)) IfExpr(-->(nestif), iftrue, iffalse) else null
      // Rules of N-Language-Extension
      case SuccExpr(t) => if (progressPossible(t) && !isFinalResult(t)) SuccExpr(-->(t)) else null
      case PredExpr(t) => if (progressPossible(t) && !isFinalResult(t)) PredExpr(-->(t)) else null
      case IsZeroExpr(t) => if (progressPossible(t) && !isFinalResult(t)) IsZeroExpr(-->(t)) else null
      // Default case
      case _ => null
    }
  }
