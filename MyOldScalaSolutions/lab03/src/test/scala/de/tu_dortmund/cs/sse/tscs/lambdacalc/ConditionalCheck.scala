...
import de.tu_dortmund.cs.sse.tscs.{FunctionTypeInformation, TypeInformations}
...
class ConditionalCheck extends AnyFlatSpec with Matchers {
  ...
  "if true then λz.z else λx.x" should "typecheck" in {
    ...
    typeInfo.get shouldEqual FunctionTypeInformation(TypeInformations.AbstractBaseType, TypeInformations.AbstractBaseType)
  }
  ...
  "λz : Bool.if z then true else false" should "typecheck" in {
    ...
    val typeInfo = parserResult.get.typecheck()
    typeInfo shouldBe a[Success[_]]
    typeInfo.get shouldEqual FunctionTypeInformation(TypeInformations.Bool, TypeInformations.Bool)
  }
